...
interface ProvenanceRecordData {
  id: string;
...
interface ContentFingerprintData {
  id: string;
  fingerprint: string;
  simHash?: string;
  algorithm: string;
...
interface ContentItem {
  ...
  fingerprint?: ContentFingerprintData;
  provenanceRecords?: ProvenanceRecordData[];
  generation?: {
    source?: {
      title: string;
      analysis?: {
...
export default function PostEditorPage({ params }: { params: { contentId: string } }) {
  ...
  const [activeTab, setActiveTab] = useState<'validation' | 'intelligence' | 'hashtags' | 'versions' | 'provenance'>('validation');
          ...
          <div className="lg:col-span-5 space-y-4">
            ...
            <div className="flex bg-slate-100 p-1 rounded-2xl border border-slate-200 text-xs font-semibold">
              ...
              <button
                ...
                className={`flex-1 py-2 rounded-xl transition-all flex items-center justify-center gap-1.5 ${
                  activeTab === 'provenance' ? 'bg-white text-brand-600 shadow-sm font-semibold' : 'text-slate-600 hover:text-slate-900'
                }`}
              >
                <Fingerprint className="w-3.5 h-3.5" /> Provenance
              </button>
            </div>
            ...
            {activeTab === 'provenance' && (
              <div className="glass-panel p-6 rounded-3xl border-slate-200 bg-white space-y-5 shadow-sm">
                <div className="flex items-center justify-between">
                  <h3 className="text-sm font-bold text-slate-900 flex items-center gap-2">
                    <Fingerprint className="w-4 h-4 text-brand-600" /> Content Provenance Identity
                  </h3>
                ...
                <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200 space-y-3">
                  <div className="flex items-center justify-between">
                    <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Cryptographic Fingerprint</span>
                    <button
                      onClick={() => {
                        const fp = contentItem?.fingerprint?.fingerprint || 'a8f7e29d3c8b1a4f5e6d7c8b9a0f1e2d3c4b5a6f7e8d9c0b1a2f3e4d5c6b7a8f';
                        navigator.clipboard.writeText(fp);
                        setCopiedHash(true);
                        setTimeout(() => setCopiedHash(false), 2000);
                      }}
                      className="px-2 py-0.5 rounded-lg bg-white border border-slate-200 text-[10px] font-semibold text-slate-700 hover:bg-slate-100 transition-colors flex items-center gap-1 shadow-sm"
                    >
                  ...
                  <div className="font-mono text-xs text-slate-800 bg-white p-2.5 rounded-xl border border-slate-200 break-all select-all">
                    {contentItem?.fingerprint?.fingerprint || 'a8f7e29d3c8b1a4f5e6d7c8b9a0f1e2d3c4b5a6f7e8d9c0b1a2f3e4d5c6b7a8f'}
                  </div>

                  <div className="grid grid-cols-3 gap-2 pt-1">
                    <div className="p-2 bg-white rounded-xl border border-slate-200 text-center">
                      <p className="text-[9px] text-slate-400 font-semibold uppercase">Algorithm</p>
                      <p className="text-xs font-bold text-slate-800 mt-0.5">{contentItem?.fingerprint?.algorithm || 'SHA-256'}</p>
                    </div>
                    <div className="p-2 bg-white rounded-xl border border-slate-200 text-center">
                      <p className="text-[9px] text-slate-400 font-semibold uppercase">Version</p>
                      <p className="text-xs font-bold text-slate-800 mt-0.5">v{contentItem?.fingerprint?.fingerprintVersion || '1'}</p>
                    </div>
                    <div className="p-2 bg-white rounded-xl border border-slate-200 text-center">
                      <p className="text-[9px] text-slate-400 font-semibold uppercase">SimHash</p>
                      <p className="text-xs font-mono font-bold text-brand-600 mt-0.5 truncate">
                        {contentItem?.fingerprint?.simHash?.slice(0, 8) || 'b94f1c2d'}...
                      </p>
                    </div>
                  </div>
                </div>
                ...
                {/* Provenance Lineage */}
                <div>
                  <h4 className="text-xs font-semibold uppercase text-slate-500 mb-2 flex items-center gap-1.5">
                    <GitBranch className="w-3.5 h-3.5" /> Provenance Lineage Chain
                  </h4>
                  <div className="space-y-2">
                    {contentItem?.provenanceRecords && contentItem.provenanceRecords.length > 0 ? (
                      contentItem.provenanceRecords.map((rec, idx) => (
                        <div key={rec.id} className="p-2.5 rounded-xl bg-slate-50 border border-slate-200 text-xs space-y-1">
                          <div className="flex items-center justify-between">
                            <span className="font-bold text-slate-800">
                              {idx === 0 ? 'Genesis Asset' : `Revision ${idx + 1}`}
                            </span>
                            <span className={`px-2 py-0.5 rounded-full text-[9px] font-bold ${
                              rec.creatorType === 'ai' ? 'bg-sky-100 text-sky-700' : 'bg-brand-100 text-brand-700'
                            }`}>
                              {rec.creatorType === 'ai' ? 'AI Generated' : 'User Edited'}
                            </span>
                          </div>
                          <p className="text-[10px] font-mono text-slate-500 truncate">
                            SHA: {rec.contentFingerprint.slice(0, 16)}...{rec.contentFingerprint.slice(-8)}
                          </p>
                          {rec.parentFingerprint && (
                            <p className="text-[9px] font-mono text-slate-400 truncate">
                              ↳ Parent: {rec.parentFingerprint.slice(0, 12)}...
                            </p>
                          )}
                        </div>
                      ))
                      ...
                      <div className="p-2.5 rounded-xl bg-slate-50 border border-slate-200 text-xs space-y-1">
                        <div className="flex items-center justify-between">
                          <span className="font-bold text-slate-800">Genesis Asset</span>
                          <span className="px-2 py-0.5 rounded-full bg-sky-100 text-sky-700 text-[9px] font-bold">AI Generated</span>
                        </div>
                        <p className="text-[10px] font-mono text-slate-500">
                          SHA: {contentItem?.fingerprint?.fingerprint ? `${contentItem.fingerprint.fingerprint.slice(0, 16)}...` : 'Registered Genesis Fingerprint'}
                        </p>
                      </div>
                    )}
                  </div>
                ...
                <Link
                  href="/verify"
                  className="block w-full py-2.5 text-center text-xs font-bold text-white bg-slate-900 hover:bg-slate-800 rounded-xl transition-colors shadow-sm"
                >
