    ...
    <div className="min-h-screen flex flex-col bg-surface-950">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 lg:px-8 py-8 space-y-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-surface-800">
          <div>
            <h1 className="text-2xl md:text-3xl font-extrabold text-white tracking-tight flex items-center gap-3">
              <LayoutTemplate className="w-7 h-7 text-brand-400" />
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-1.5 overflow-x-auto w-full sm:w-auto p-1 bg-surface-900 rounded-2xl border border-surface-800">
            {CATEGORIES.map((cat) => (
              <button
                key={cat}
          ...
          <div className="relative w-full sm:w-72">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2 rounded-xl bg-surface-900 border border-surface-800 text-white placeholder-surface-500 text-xs focus:outline-none focus:border-brand-500"
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {filteredTemplates.map((tpl, i) => (
              <div
                key={tpl.id || i}
                className="glass-panel glass-panel-hover p-6 rounded-3xl border-surface-800 flex flex-col justify-between"
              >
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    {tpl.isSystem && (
                      <span className="text-[10px] text-accent-cyan font-semibold flex items-center gap-1">
                        <Sparkles className="w-3 h-3" /> System Default
                      </span>
                    )}
                  </div>
                ...
                <div className="mt-6 pt-4 border-t border-surface-800/80 flex items-center justify-between">
                  <span className="text-[10px] text-surface-500">Tone: {tpl.defaultTone || 'Professional'}</span>
                  <Link
                    href="/create"
                    className="px-3.5 py-1.5 rounded-xl bg-brand-600/20 hover:bg-brand-600 text-brand-300 hover:text-white text-xs font-semibold transition-colors flex items-center gap-1"
                  >
        ...
        {isModalOpen && (
          <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
            <div className="glass-panel p-6 rounded-3xl border-surface-800 max-w-md w-full space-y-4">
              <h3 className="text-lg font-bold text-white">Save Custom Template</h3>

              <form onSubmit={handleCreateTemplate} className="space-y-4">
                <div>
                  <label className="block text-xs font-semibold text-surface-300 mb-1">Template Name</label>
                  <input
                    type="text"
                    required
                    value={name}
                    onChange={(e) => setName(e.target.value)}
                    className="w-full px-4 py-2.5 rounded-xl bg-surface-900 border border-surface-800 text-white text-xs focus:outline-none focus:border-brand-500"
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full p-2.5 rounded-xl bg-surface-900 border border-surface-800 text-white text-xs focus:outline-none focus:border-brand-500"
                    placeholder="Brief description of format..."
                  />
                </div>

                <div>
                  <label className="block text-xs font-semibold text-surface-300 mb-1">Prompt Instructions</label>
                  <textarea
                    rows={3}
                    required
